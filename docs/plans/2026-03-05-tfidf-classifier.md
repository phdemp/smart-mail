# TF-IDF Email Classifier Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace the slow Ollama LLM path in the FastAPI classifier with a TF-IDF + LogisticRegression model that classifies emails in <5ms instead of ~70 seconds.

**Architecture:** Add TF-IDF as Tier 3 between the rule_engine and LLM in the FastAPI pipeline. A training script generates labeled data from synthetic examples + real DB emails (run rules on them), trains a sklearn Pipeline, and saves it to disk. FastAPI loads the model at startup and uses it with a confidence threshold of 0.75; emails below threshold fall back to LLM. The model can be retrained anytime as new data accumulates.

**Tech Stack:** scikit-learn 1.x (TfidfVectorizer + LogisticRegression), joblib, numpy — all installed in the existing venv at `/opt/intellimail-classifier/venv/`. Python 3.9.25. FastAPI stays unchanged except for the new tier insertion.

**Target server:** `10.11.13.237` (Rocky Linux 9.7)
**FastAPI code:** `/opt/intellimail-classifier/api/main.py`
**Venv:** `/opt/intellimail-classifier/venv/`
**Data dir:** `/opt/intellimail-classifier/data/`
**DB:** `/home/AjayData/xgen-intel/intellimail/intellimail.db`

---

## Task 1: Install sklearn dependencies in venv

**Files:**
- No file changes — just package installation

**Step 1: Install packages via venv pip**

Run on staging via SSH:
```bash
/opt/intellimail-classifier/venv/bin/pip install scikit-learn joblib numpy
```

Expected output: `Successfully installed scikit-learn-X.X joblib-X.X numpy-X.X`

**Step 2: Verify installation**

```bash
/opt/intellimail-classifier/venv/bin/python -c "import sklearn; import joblib; import numpy; print('OK', sklearn.__version__)"
```
...
## Task 2: Create training data generator + trainer script

**Files:**
- Create: `/opt/intellimail-classifier/data/train_tfidf.py`

This script:
1. Reads existing `training.jsonl` (8 synthetic examples)
2. Reads real emails from `intellimail.db` and applies rule-engine logic to label them
3. Combines all data, trains TF-IDF + LogisticRegression pipeline
4. Saves model to `/opt/intellimail-classifier/data/tfidf_model.joblib`
...
# ── Rule engine labels (mirrors classifier.js logic) ─────────────────────────
def rule_label(subject, from_addr, body_preview):
    ...
    return None  # no confident label — skip for training
...
# ── Synthetic examples from training.jsonl ────────────────────────────────────
def load_jsonl():
    texts, labels = [], []
...
# ── Extended synthetic training data (generalizable patterns) ─────────────────
SYNTHETIC = [
    # meeting_request (20 examples)
...
def load_db_emails():
    """Load emails from DB, label with rule engine (high-confidence only)."""
    texts, labels = [], []
...
def train():
    ...
    # Source 1: existing training.jsonl
    t1, l1 = load_jsonl()
    print(f"  training.jsonl: {len(t1)} examples")

    # Source 2: extended synthetic examples
    t2, l2 = load_synthetic()
    print(f"  Synthetic examples: {len(t2)}")

    # Source 3: rule-labeled DB emails
    ...
    test_cases = [
        ...
        ("Your HDFC credit card statement is ready", "statements@hdfcbank.com", "HDFC", "Amount due Rs 12000 by March 15.", "financial"),
    ...
    for subj, frm, name, prev, expected in test_cases:
        ...
        print(f"  [{status}] '{subj[:40]}' => {pred} ({proba:.2f}) [expected: {expected}]")
...
```bash
cd /opt/intellimail-classifier
/opt/intellimail-classifier/venv/bin/python data/train_tfidf.py
```

Expected output:
```
Loading training data...
  training.jsonl: 8 examples
  Synthetic examples: 130
  Found N emails in DB
  Rule-labeled X emails from DB
Total training examples: ~140-200
Cross-validation accuracy: 0.85 (+/- 0.05)
Model saved to /opt/intellimail-classifier/data/tfidf_model.joblib
Sanity checks:
  [OK] 'Meeting scheduled: Zoom call Friday 3pm' => meeting_request (0.92)
  [OK] 'Your HDFC credit card statement is ready' => financial (0.96)
  [OK] 'IndiGo booking confirmation PNR ABCXYZ' => travel (0.98)
Training complete.
```

---

## Task 3: Integrate TF-IDF into FastAPI pipeline

**Files:**
- Modify: `/opt/intellimail-classifier/api/main.py`

**Changes:** Add TF-IDF as a new pipeline stage between `rule_engine` and `call_llm`. Confidence threshold: 0.75 — above it, use TF-IDF result; below it, fall through to LLM.

**Step 1: Add TF-IDF loading at startup**

After the `S = St()` class definition (around line 35), add to the `St` class:
```python
class St:
    ...existing fields...
    tfidf_model = None          # sklearn Pipeline, loaded at startup
```

In the `lifespan` function, after `S.sem = asyncio.Semaphore(WORKERS)`, add:
```python
    # Load TF-IDF model
    try:
        import joblib
        model_path = Path("/opt/intellimail-classifier/data/tfidf_model.joblib")
        if model_path.exists():
            S.tfidf_model = joblib.load(str(model_path))
            log.info("tfidf_loaded", path=str(model_path))
        else:
            log.warning("tfidf_model_not_found", path=str(model_path))
    except Exception as ex:
        log.warning("tfidf_load_failed", err=str(ex))
```

Also add `from pathlib import Path` at the top imports if not present.

**Step 2: Add tfidf_classify function**

Add this function after `rule_engine()` and before `call_llm()`:

```python
TFIDF_THRESHOLD = 0.75

def tfidf_classify(e: EIn):
    """Tier 3: TF-IDF sklearn classifier. Returns resolved dict or None."""
...
**Step 3: Insert TF-IDF into the pipeline orchestrator**

In the `classify()` async function, after the `rule_engine` block and before the LLM call, add:

```python
        # Tier 3: TF-IDF classifier (fast, CPU, <5ms)
        tf = tfidf_classify(hint if re and not re.get("resolved") else email)
        if tf and tf.get("resolved"):
            S.stages["tfidf"] = S.stages.get("tfidf", 0) + 1
            return _mk(email, tf, t0)
```

Also add `"tfidf": 0` to the `stages` dict initialization in `St`:
```python
stages = {"hard_filter":0,"rule_engine":0,"tfidf":0,"local_llm":0,"error":0}
```

**Step 4: Restart the FastAPI service**

```bash
systemctl restart intellimail-classifier
# or if using uvicorn directly:
pkill -f "uvicorn main:app" && cd /opt/intellimail-classifier/api && nohup /opt/intellimail-classifier/venv/bin/uvicorn main:app --host 0.0.0.0 --port 8765 --workers 1 > /opt/intellimail-classifier/logs/api.log 2>&1 &
```

---

## Task 4: Test and validate performance

**Step 1: Test TF-IDF path directly**

Run on staging:
```bash
time curl -s -X POST http://localhost:8765/classify \
  -H "Content-Type: application/json" \
  -d '{"subject":"Meeting request: Zoom call Friday 3pm","from_address":"manager@company.com","preview":"Can we hop on a quick Zoom call Friday?","email_id":"test1"}'
```

Expected: response in <100ms, `stage` field = `"tfidf:meeting_request"`

**Step 2: Test financial email**

```bash
time curl -s -X POST http://localhost:8765/classify \
  -H "Content-Type: application/json" \
  -d '{"subject":"Your HDFC credit card statement","from_address":"statements@hdfcbank.com","preview":"Amount due Rs 24500 by March 15","email_id":"test2"}'
```

Expected: <100ms, `stage` = `"tfidf:financial"` or `"rule_engine:statement"`

**Step 3: Check health endpoint for stage distribution**

```bash
curl -s http://localhost:8765/health | python3 -m json.tool | grep -A5 stage
```

Expected: shows `tfidf` stage count increasing, `local_llm` decreasing.

**Step 4: Run 10 real emails through the API**

```bash
sqlite3 /home/AjayData/xgen-intel/intellimail/intellimail.db \
  "SELECT id, subject, from_address, from_name, substr(body_text,1,400) FROM emails LIMIT 10" | \
while IFS='|' read id subj from name preview; do
  echo "--- Email $id ---"
  time curl -s -X POST http://localhost:8765/classify \
    -H "Content-Type: application/json" \
    -d "{\"subject\":\"$subj\",\"from_address\":\"$from\",\"from_name\":\"$name\",\"preview\":\"$preview\",\"email_id\":\"$id\"}" \
    | python3 -c "import sys,json; d=json.load(sys.stdin); print(f'  category={d[\"category\"]} stage={d[\"stage\"]} ms={d[\"processing_ms\"]}')"
done
```

Expected: most emails now showing `stage=tfidf:*` or `stage=rule_engine:*`, processing_ms < 100.

---

## Task 5: Reclassify existing DB emails

Now that classification is fast, reclassify all emails that currently have no classification or were classified as 'other' via LLM.

**Step 1: Trigger reclassification via the Node.js app**

The Node.js server has `classifyAllUnclassified()` which runs on startup. Restart PM2 to trigger it:

```bash
pm2 restart intellimail
```

**Step 2: Verify in DB after 30 seconds**

```bash
sqlite3 /home/AjayData/xgen-intel/intellimail/intellimail.db \
  "SELECT category, COUNT(*) FROM classifications GROUP BY category ORDER BY COUNT(*) DESC"
```

Expected: categories distributed across all 8 types, not all 'other'.

**Step 3: Check app logs**

```bash
pm2 logs intellimail --lines 50
```

Expected: no classification errors, no timeout errors.

---

## Done: Verify speed improvement

```bash
# Before: LLM path ~70,000ms
# After: TF-IDF path <5ms (14,000x faster)

curl -s http://localhost:8765/health | python3 -c "
import sys, json
h = json.load(sys.stdin)
s = h['stats']
print(f'Total requests: {s[\"requests_total\"]}')
print(f'Stage counts: {s[\"stage_counts\"]}')
